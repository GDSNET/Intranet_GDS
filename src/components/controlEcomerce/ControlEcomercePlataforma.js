...
export default class ControlEcomerceSalas extends Component {
  ...
  funExecutar(semana, id_sala, id_plataforma){
    const parametros = {
      "parametros": semana + ',' + id_sala + ',' + id_plataforma,
      "reporte": "ecom-ctrl-categoria"
      }
...
<View  style={styles.containerfila} key={'v'+ i}> 
  <View  style={[styles.fila, {background: 'linear-gradient(to right , #231f20, #454041)'}]} key={'v'+ i}> 
          ...
          <Text style={styles.txt_grid}>id Plataforma: {value.id_plataforma}</Text>
          </View>  
          <View style={styles.view_grid_grande}>
          <Text style={styles.txt_grid}>Plataforma: {value.desc_plataforma}</Text>
          </View>  
          ...
          <View style={styles.view_grid_boton}>
              <TouchableOpacity style={styles.btnGO} onPress={()=>{this.funExecutar(value.id_tie_semana, value.id_sala, value.id_plataforma)}}>
